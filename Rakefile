...
namespace :homebrew do
  ...
  task :commit do
    Dir.chdir(HOMEBREW_FORMULAE_DIR) do
      `git add Formula/templater.rb`
      `git commit -m "templater: Release version #{Templater::VERSION}"`
    end
  end
  ...
  namespace :formula do
    ...
    task :build do
      formula = File.read('homebrew/templater.rb')
      formula.gsub!('__VERSION__', Templater::VERSION)
      formula.gsub!('__SHA__', `shasum #{GH_PAGES_DIR}/Templater-#{Templater::VERSION}.tar.gz`.split.first)
      File.write("#{HOMEBREW_FORMULAE_DIR}/Formula/templater.rb", formula)
    end
  end
end
...
namespace :tarball do
  ...
  task :checkout do
    `git clone --branch gh-pages git@github.com:maschall/templater.git #{GH_PAGES_DIR}`
  end

  desc 'Move tarball into gh-pages'
  task :move do
    FileUtils.mv("pkg/Templater-#{Templater::VERSION}.tar.gz", GH_PAGES_DIR)
  end

  desc 'Check in the new tarball'
  task :commit do
    Dir.chdir(GH_PAGES_DIR) do
      `git add Templater-#{Templater::VERSION}.tar.gz`
      `git commit -m "Release version #{Templater::VERSION}"`
    end
  end
  ...
  Rake::PackageTask.new('Templater', Templater::VERSION) do |p|
    p.need_tar_gz = true
